...
public:
	MyContainer(int cnt, int val) // 생성자
	{
		cout << "생성자 호출" << endl;
	...
	MyContainer(const MyContainer& copy) // 복사 생성자
	{
		cout << "복사 생성자 호출" << endl;
	...
	MyContainer(MyContainer&& move) // 이동 생성자
	{
		cout << "이동 생성자 호출" << endl;
		bucket=&(move.v);
		count = move.count;
	...
	MyContainer operator = (MyContainer&& move)  // 이동 대입 연산자
	{
